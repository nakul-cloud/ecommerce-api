...
| **pydantic-settings** | Configuration | Loads and validates settings from `.env` dynamically with type checks |
| **passlib[bcrypt]** | Security | Standard password hashing library to secure credentials |
| **PyJWT** | Tokens | Python implementation to generate and verify JWTs safely |
| **slowapi** | Rate Limiting | Implements IP-based request rate limiting for brute-force defense |
| **secure** | Secure Headers | Injects standard security headers to prevent XSS, clickjacking, and mime-sniffing |
